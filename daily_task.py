...
def git_push():
    ...
    # Git add
    subprocess.run([GIT_EXEC, 'add', 'data/'], cwd=GIT_DIR, capture_output=True)
    
    # Git status
    result = subprocess.run([GIT_EXEC, 'status', '--porcelain'], cwd=GIT_DIR, capture_output=True, text=True)
    ...
    subprocess.run([GIT_EXEC, 'commit', '-m', f'更新行业数据 {today}'], cwd=GIT_DIR, capture_output=True)
    
    # Git push
    push_result = subprocess.run([GIT_EXEC, 'push'], cwd=GIT_DIR, capture_output=True, text=True)
...
def main():
    ...
    if git_push():
        ...
        msg += f"\n\n🌐 查看网站: https://egeh5romk547.space.minimaxi.com"
        
        send_telegram_message(msg)
    else:
        send_telegram_message("⚠️ 数据已更新，但GitHub推送失败")
